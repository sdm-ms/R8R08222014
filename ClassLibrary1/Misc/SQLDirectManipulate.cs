...
namespace ClassLibrary1.Misc
{
    ...
    {
        public bool HasSinglePrimaryKeyField; // if not, we must have a composite primary key (which we assume is made up by the data itself)
    ...
    public class SQLInfoForCellsInRow_MainAndSecondaryTables
    {
        ...
        private void AddToAppropriateSubgroup(SQLCellInfo newItem, SQLUpdateInfoTableSpecification tableSpec)
        {
            ...
            if (match != -1)
            {
                ...
                subgroup.SQLUpdateInfos[match] = newItem;
                newItem.RowInfo = subgroup;
                SQLUpdateInfos.Remove(itemToRemove);
            }
            else
        ...
        {
            if (tableSpec.IsMainTable && tableSpec.HasSinglePrimaryKeyField && newItem.Fieldname == tableSpec.PrimaryKeyFieldName && newItem.Rownum != null)
            {
        ...
        public void EliminateOverridenUpdates()
        {
            Subgroups.ForEach(x => x.EliminateOverridenUpdates());
        }
    }
    ...
    public class SQLInfoForCellsInRow_SingleTable
    {
        ...
        public SQLInfoForCellsInRow_SingleTable(SQLUpdateInfoTableSpecification tableSpec)
        {
            ...
            else if (!TableSpec.HasSinglePrimaryKeyField)
        ...
        public void Add(SQLCellInfo newItem)
        {
            ...
            else if (TableSpec.HasSinglePrimaryKeyField && !newItem.DataIsAlreadyInDatabase)
        ...
        public void GetUpdateCommand(out string updateCommand, out List<SQLCellInfo> updateInfos)
        {
            ...
            updateInfos = updateCommands.OrderBy(x => x.Fieldname == "ID").ToList(); // put ID field last
        }
    }
    ...
    {
        public static void EliminateOverridenUpdates(this List<SQLCellInfo> updates)
        {
            ...
            {
                SQLCellInfo update = updates[itemNum];
                string key = update.Paramname() + update.Delete.ToString();
                if (!itemsProcessed.ContainsKey(key))
                    itemsProcessed.Add(key, true);
                else
...
";
            ...
            if (fieldNames.Contains("ID"))
                matchingStatement = "tbl.ID = upsert2.ID";
            else
